...
class PiperNarrator(private val context: Context) {
    ...
    suspend fun prepare(label: String, mystery: Boolean, onProgress: (String) -> Unit = {}) {
                    ...
                    config = OfflineTtsConfig(
                        model = OfflineTtsModelConfig(
                            vits = OfflineTtsVitsModelConfig(
                                ...
                                noiseScale = 0.667f,
                                noiseScaleW = 0.8f,
                                lengthScale = if (mystery) 1.15f else 1.0f,
                            ),
                            numThreads = 2,
                            debug = false,
                            provider = "cpu",
                        ...
                        maxNumSentences = 1,
                        silenceScale = if (mystery) 0.35f else 0.25f,
                    ),
                )
            }
        }
        status = TtsStatus(
            ...
            voiceLabel = label,
            message = if (mystery) {
                "Offline · argentino · narración de misterio"
            } else {
                "Offline · acento argentino (Piper)"
            },
        )
        // Prueba corta
        withContext(Dispatchers.IO) {
            val audio = requireTts().generate(text = "Listo.", sid = 0, speed = if (mystery) 0.88f else 0.95f)
    ...
    suspend fun speak(
        ...
        val chunks = StoryChunks.split(text, maxChars = 380)
        if (chunks.isEmpty()) {
            withContext(Dispatchers.Main) { onDone() }
            return
        }
        val speed = (rate.coerceIn(0.7f, 1.35f) * if (mystery) 0.88f else 0.95f)
            .coerceIn(0.55f, 1.4f)
        ...
        withContext(Dispatchers.IO) {
            for ((index, chunk) in chunks.withIndex()) {
                ...
                val audio = try {
                    ...
                    throw IllegalStateException("Error de voz offline: ${e.message}", e)
                }
                if (session.get() != my) return@withContext
                ...
                try {
                    ...
                    if (session.get() == my && index < chunks.lastIndex) delay(60)
                } finally {
                    wav.delete()
                }
            }
