...
class PiperNarrator(private val context: Context) {
    ...
    suspend fun speak(
        ...
        val chunks = StoryChunks.splitPassages(prepared)
        if (chunks.isEmpty()) {
            withContext(Dispatchers.Main) { onDone() }
            return
        }
        val speed = (HumanPacing.PIPER_BASE_SPEED * rate.coerceIn(0.85f, 1.15f)).coerceIn(0.85f, 1.05f)

        withContext(Dispatchers.IO) {
            coroutineScope {
                val jobs = MutableList(chunks.size) { null as kotlinx.coroutines.Deferred<com.k2fsa.sherpa.onnx.GeneratedAudio?>? }
                jobs[0] = async { runCatching { engine.generate(text = chunks[0], sid = 0, speed = speed) }.getOrNull() }
                if (chunks.size > 1) {
                    jobs[1] = async { runCatching { engine.generate(text = chunks[1], sid = 0, speed = speed) }.getOrNull() }
                }
                for (index in chunks.indices) {
                    ...
                    val audio = jobs[index]?.await()
                        ?: engine.generate(text = chunks[index], sid = 0, speed = speed)
                    jobs[index] = null
